<template>
  <div class="server-overview">
    ...
    <el-card v-if="data.config.autoTransportEnabled" class="config-card" shadow="hover">
      <template #header>
        <div class="card-header">
          <span class="card-title">Auto Transport</span>
          <el-tag size="small" type="success">{{ data.config.transportProtocol }}</el-tag>
        </div>
      </template>
      <div class="auto-grid">
        <div class="config-item">
          ...
          <div class="tag-list">
            <el-tag
              v-for="protocol in data.config.autoTransportProtocols"
              :key="protocol"
              size="small"
              type="info"
            >
        ...
        <div class="config-item">
          ...
          <span class="config-value">
            {{ data.status.autoNegotiationSuccess }} / {{ data.status.autoNegotiationFailure }}
          </span>
        </div>
        <div class="config-item">
...
const selectionRows = computed(() => sortedRows(data.value.status.autoTransportSelections || {}))
const onlineRows = computed(() => sortedRows(data.value.status.autoTransportClientCounts || {}))
const switchRows = computed(() => sortedRows(data.value.status.autoTransportSwitchCounts || {}))

const illegalSelectionTotal = computed(() => {
  return Object.values(data.value.status.autoTransportIllegalSelections || {}).reduce(
    (sum, value) => sum + value,
    0,
  )
})
...
const switchTotal = computed(() => {
  return Object.values(data.value.status.autoTransportSwitchCounts || {}).reduce(
    (sum, value) => sum + value,
    0,
  )
})
